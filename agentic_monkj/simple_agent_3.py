...
"""def query(question, max_turns=5):
    ...
    while i < max_turns:
        ...
        if actions:
            ...
            print(" __running {} {}".format(action, action_input))
...
def query_interactive():
    bot = Agent(prompt)
    max_turns = int(input("Enter the max turns: "))
    i = 0
    ...
    while i < max_turns:
        ...
        if actions:
            action, action_input = actions[0].groups()
            if action not in known_actions:
                print("Unknown action: {action}: {action_input}".format(action, action_input))
                continue
            print(f" __running {action} {action_input}")
            observation = known_actions[action](action_input)
            print("Observation:", observation)
